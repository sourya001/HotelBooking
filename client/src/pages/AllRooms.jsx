...
const AllRooms = () => {
  ...
  const filteredRooms = useMemo(() => {
    return rooms.filter(room => matchesPriceRange(room) && matchesRoomType(room) && filteredDestination(room)).sort(sortRooms);
        ...
        >
          <div className="px-5 pt-5">
            ...
            {roomTypes.map((roomType, index) => (
              <CheckBox key={index} label={roomType} selected={selectedFilters.roomTypes.includes(roomType)} onChange={(checked) => handleFilterChange(checked, roomType, "roomTypes")} />
            ))}
          </div>
          <div className="px-5 pt-5">
